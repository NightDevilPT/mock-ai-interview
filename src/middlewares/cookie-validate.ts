...
type Handler = (request: NextRequest, payload?: TokenPayload) => Promise<NextResponse>;
...
export function withTokenValidation(handler: Handler): Handler {
  return async (request: NextRequest): Promise<NextResponse> => {
    try {
      ...
      if (accessToken) {
        const accessTokenValid = jwtService.verifyToken(accessToken, "access");
        if (accessTokenValid) {
          payload = jwtService.verifyToken(accessToken, "access") as TokenPayload;
      ...
      if (shouldRefresh && refreshToken) {
        const refreshTokenValid = jwtService.verifyToken(refreshToken, "refresh");

        if (refreshTokenValid) {
          // Get payload from refresh token
          payload = jwtService.verifyToken(refreshToken, "refresh") as TokenPayload;
